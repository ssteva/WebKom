...
{
    [Route("[controller]")]
    public class TokenOldController : Controller
    {
        ...
        public TokenOldController(TokenAuthOptions tokenOptions, UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager, ISession session)
        {
        ...
        [Route("[action]")]
        //[Authorize("Bearer")]
        public async Task<ActionResult> RefreshToken([FromBody] RefreshTokenData refresh)
        {
            var re = Request;
        ...
        {
            var user = await _userManager.FindByNameAsync(req.username) ?? await _userManager.FindByEmailAsync(req.username);
            ...
            var logovan = await _userManager.CheckPasswordAsync(user, req.password);
